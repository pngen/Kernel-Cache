// example 04: shape-specialized artifacts coexist.
...
int main(){
  ...
  for (auto s : {128,256,512}){ KernelLookupRequest r; r.key=key(s); r.device.vendor=DeviceVendor::CPU; r.desired_tier=ResidencyTier::HostResident; r.allow_miss_build=true;
    auto l=cache.acquire(r); if(l.ok()){ std::cout<<"shape "<<s<<" -> "<<l.value().artifact_id().str().substr(0,8)<<"\n"; l.value().release(); } }
  std::cout<<"artifacts="<<cache.snapshot().artifact_count<<"\n";
  return 0;
}